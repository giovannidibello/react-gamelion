// GameCard.jsx

import { Link } from "react-router-dom"

const GameCard = (props) => {

    const coverBaseUrl = import.meta.env.VITE_COVER_API_URL;
    const isDetail = props.isDetail || false;
    const { id, title, cover_image, platforms } = props.gameProp;

    const platformIconsById = {
        1: <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0 0 48 48">
            <path fill="#0076d7" d="M2,30H0v-4c0-1.654,1.346-3,3-3h9.5c0.827,0,1.5-0.673,1.5-1.5S13.327,20,12.5,20H0v-2h12.5	c1.93,0,3.5,1.57,3.5,3.5S14.43,25,12.5,25H3c-0.551,0-1,0.448-1,1V30z"></path><path fill="#0076d7" d="M20,30h-7v-2h7c0.551,0,1-0.448,1-1v-6c0-1.654,1.346-3,3-3h7v2h-7c-0.551,0-1,0.448-1,1v6	C23,28.654,21.654,30,20,30z"></path><path fill="#0076d7" d="M44.5,30H33v-2h11.5c0.827,0,1.5-0.673,1.5-1.5S45.327,25,44.5,25h-8.929	C34.153,25,33,23.847,33,22.429V18h14v2H35v2.429C35,22.744,35.256,23,35.571,23H44.5c1.93,0,3.5,1.57,3.5,3.5S46.43,30,44.5,30z"></path>
        </svg>,
        2: <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0 0 50 50">
            <path d="M 25 2 C 20.709 2 16.618563 3.2007813 13.226562 5.3007812 C 13.007563 5.3737813 12.629156 5.6617969 12.285156 5.9667969 C 15.330156 3.5477969 22.371734 8.3929375 24.427734 9.8359375 C 24.773734 10.078938 25.228219 10.078938 25.574219 9.8359375 C 27.630219 8.3929375 34.671797 3.5467969 37.716797 5.9667969 C 37.372797 5.6617969 36.993391 5.3737813 36.775391 5.3007812 C 33.382391 3.2007813 29.291 2 25 2 z M 11 8 C 9.403 8 8.2363281 9.3007812 8.2363281 9.3007812 C 4.4443281 13.400781 2.0507812 18.9 2.0507812 25 C 2.0507812 37.7 12.328 48 25 48 C 31.685 48 37.771891 45.1 41.962891 40.5 C 41.962891 40.5 41.464094 37.499609 38.371094 33.099609 C 34.912094 27.882609 27.905109 21.311922 25.662109 19.294922 C 25.279109 18.950922 24.708125 18.952781 24.328125 19.300781 C 22.638125 20.847781 18.277406 25.177781 17.316406 26.300781 C 15.021406 28.700781 8.6353281 36.299609 8.2363281 40.599609 C 8.2363281 40.599609 6.8386406 37.200391 9.9316406 29.400391 C 11.856641 24.714391 17.835375 17.747984 20.734375 14.708984 C 21.119375 14.305984 21.110125 13.669109 20.703125 13.287109 C 19.743125 12.388109 17.505281 10.812609 15.488281 9.5996094 C 14.092281 8.6996094 12.497 8.1 11 8 z M 38.689453 8 C 38.036453 8 33.794078 9.3428281 29.330078 13.298828 C 28.908078 13.672828 28.891203 14.325469 29.283203 14.730469 C 30.900203 16.401469 35.322656 20.681391 37.972656 24.900391 C 41.265656 30.300391 43.2605 34.6 42.0625 40.5 C 45.7545 36.4 48.050781 31 48.050781 25 C 47.950781 19 45.655281 13.500391 41.863281 9.4003906 C 41.763281 9.3003906 41.663453 9.1996094 41.564453 9.0996094 C 40.766453 8.1996094 39.587453 8 38.689453 8 z"></path>
        </svg>,
        3: <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0 0 48 48">
            <path fill="#efebe9" d="M22,7v34l-8,0c0,0-6-2-6-7c0-3-1-19,0-21s3-5,6-6C15.897,6.368,22,7,22,7z"></path><path fill="none" stroke="#ff3d00" stroke-miterlimit="10" stroke-width="2" d="M17,7h5v34l-6,0c-4.401,0-8-3.33-8-8.053v-17	C8,10.27,11.719,7,17,7z"></path><path fill="#ff3d00" d="M30,6h-5v36h5c5,0,10-4,10-9V16C40,10,35.658,6,30,6z M32.5,31c-1.934,0-3.5-1.566-3.5-3.5	c0-1.934,1.566-3.5,3.5-3.5c1.934,0,3.5,1.566,3.5,3.5C36,29.434,34.434,31,32.5,31z"></path><circle cx="15.5" cy="16.5" r="3.5" fill="#ff3d00" fill-rule="evenodd" clip-rule="evenodd"></circle>
        </svg>,
        4: <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0 0 50 50">
            <path d="M 1 19 A 1.0001 1.0001 0 1 0 1 21 L 12.5 21 C 13.340812 21 14 21.659188 14 22.5 C 14 23.340812 13.340812 24 12.5 24 L 3 24 C 1.3550302 24 0 25.35503 0 27 L 0 30 A 1.0001 1.0001 0 1 0 2 30 L 2 27 C 2 26.43497 2.4349698 26 3 26 L 12.5 26 C 14.28508 26 15.719786 24.619005 15.921875 22.884766 A 1.0001 1.0001 0 0 0 16 22.5 C 16 20.578812 14.421188 19 12.5 19 L 1 19 z M 26 19 C 24.35503 19 23 20.35503 23 22 L 23 28 C 23 28.56503 22.56503 29 22 29 L 16 29 A 1.0001 1.0001 0 1 0 16 31 L 22 31 C 23.64497 31 25 29.64497 25 28 L 25 22 C 25 21.43497 25.43497 21 26 21 L 32 21 A 1.0001 1.0001 0 1 0 32 19 L 26 19 z M 46.970703 19 A 1.0001 1.0001 0 0 0 46.503906 19.130859 L 32.503906 27.130859 A 1.0001 1.0001 0 0 0 33 29 L 46 29 L 46 30 A 1.0001 1.0001 0 1 0 48 30 L 48 29 L 49 29 A 1.0001 1.0001 0 1 0 49 27 L 48 27 L 48 20 A 1.0001 1.0001 0 0 0 46.970703 19 z M 46 21.724609 L 46 27 L 36.767578 27 L 46 21.724609 z"></path>
        </svg>,
        5: <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0 0 48 48">
            <radialGradient id="2Jwv7LuZrao5GsFL~0k11a_wlFMPtzt97an_gr1" cx="24" cy="18.435" r="40.735" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#dfe9f2"></stop><stop offset=".543" stop-color="#d6e0e9"></stop><stop offset="1" stop-color="#bfc8d1"></stop></radialGradient><path fill="url(#2Jwv7LuZrao5GsFL~0k11a_wlFMPtzt97an_gr1)" d="M44,24c0,4.37-1.4,8.41-3.79,11.7c-0.66,0.93-1.41,1.8-2.23,2.59C34.37,41.82,29.44,44,24,44	s-10.37-2.18-13.98-5.71C9.2,37.5,8.45,36.63,7.79,35.7C5.4,32.41,4,28.37,4,24c0-5.69,2.37-10.81,6.18-14.45	c1-0.96,2.1-1.81,3.28-2.54C16.52,5.1,20.13,4,24,4s7.48,1.1,10.54,3.01c1.18,0.73,2.28,1.58,3.28,2.54C41.63,13.19,44,18.31,44,24z"></path><radialGradient id="2Jwv7LuZrao5GsFL~0k11b_wlFMPtzt97an_gr2" cx="23.891" cy="14.717" r="19.79" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#4caf50"></stop><stop offset=".489" stop-color="#4aaf50"></stop><stop offset=".665" stop-color="#43ad50"></stop><stop offset=".79" stop-color="#38aa50"></stop><stop offset=".892" stop-color="#27a550"></stop><stop offset=".978" stop-color="#11a050"></stop><stop offset="1" stop-color="#0a9e50"></stop></radialGradient><path fill="url(#2Jwv7LuZrao5GsFL~0k11b_wlFMPtzt97an_gr2)" d="M40.21,35.7c-0.66,0.93-1.41,1.8-2.23,2.59c-0.07-0.61-0.31-2.47-0.98-3.79	c-1.5-3-8-11-13-14.5c-5,3.5-11.5,11.5-13,14.5c-0.67,1.32-0.91,3.18-0.98,3.79C9.2,37.5,8.45,36.63,7.79,35.7	C7.76,35.08,7.8,34.2,8,33c0.74-4.47,8.5-16,11-18c-3.4-3.4-6.8-4.91-8.82-5.45c1-0.96,2.1-1.81,3.28-2.54	C19.15,7.1,22.05,9.03,24,10c1.95-0.97,4.85-2.9,10.54-2.99c1.18,0.73,2.28,1.58,3.28,2.54C35.8,10.09,32.4,11.6,29,15	c2.5,2,10.26,13.53,11,18C40.2,34.2,40.24,35.08,40.21,35.7z"></path>
        </svg>,
        6: <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0 0 48 48">
            <path fill="#00b0ff" d="M20 25.026L5.011 25 5.012 37.744 20 39.818zM22 25.03L22 40.095 42.995 43 43 25.066zM20 8.256L5 10.38 5.014 23 20 23zM22 7.973L22 23 42.995 23 42.995 5z"></path>
        </svg>

    }

    return (

        <div className="card">
            <div className="top-section">
                <div className="icons">
                    <div className="social-media">
                        {platforms.map(platform => platformIconsById[platform.id] || null)}
                    </div>
                </div>
                <img src={`${coverBaseUrl}${cover_image}`} alt={title} />
            </div>
            <div className="bottom-section">
                <span className="title">{title}</span>
            </div>
        </div>
    );
};

export default GameCard